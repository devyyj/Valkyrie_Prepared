...
POJO는 Plain Old Java Object의 약자로, 스프링 프레임워크에서 객체 지향 프로그래밍의 핵심 원칙을 따르는 **일반적인 자바 객체**를 의미합니다.

POJO는 자바 클래스를 만들 때, **특정한 인터페이스를 구현하거나 특정한 클래스를 상속받지 않으며, 특정한 규약이나 제약 조건을 강제하지 않는다는 특징**을 가지고 있습니다. 이러한 POJO의 특징으로 인해, 스프링
