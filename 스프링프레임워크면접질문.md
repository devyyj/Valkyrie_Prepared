...
스프링 프레임워크(Spring Framework)는 자바 기반의 오픈 소스 애플리케이션 **프레임워크**입니다. 스프링은 애플리케이션 개발에 필요한 많은 기능을 제공하며, 웹 개발, 데이터베이스 연동, 트랜잭션
처리, 보안
...
스프링 AOP는 **프록시 패턴과 리플렉션을 사용하여 구현**됩니다. 프록시 패턴을 사용하여 원본 객체를 대신하여 Advice를 적용한 프록시 객체를 생성하며, 리플렉션을 사용하여 프록시 객체에서 Advice를
적용할
메서드를 동적으로 호출합니다.
...
프록시 패턴과 리플렉션은 모두 객체 지향 프로그래밍에서 사용되는 기술로, 객체를 다루는 방법과 객체의 동작을 제어하는 방법을 다룹니다.

* 프록시 패턴: 객체를 대신하여 특정 동작을 수행하는 객체를 만들어, 클라이언트와 객체 사이의 중간 매개체 역할을 수행하는 패턴입니다. 프록시 객체는 실제 객체를 감싸고, 클라이언트가 객체에 접근할 때 프록시
  객체가 접근을 대신 처리합니다. 프록시 객체는 실제 객체와 같은 인터페이스를 구현하며, 클라이언트는 실제 객체인지 프록시 객체인지 구분하지 않고 호출할 수 있습니다. 프록시 패턴은 객체의 접근 제어, 객체 생성
  비용 절감, 객체 지연 로딩 등의 용도로 사용됩니다.

* 리플렉션: 런타임 시점에 클래스의 정보를 분석하여, 클래스의 인스턴스를 생성하고, 메소드를 호출하고, 필드에 접근하는 등의 작업을 수행하는 기술입니다. 리플렉션을 사용하면 클래스 정보를 동적으로 분석하여 객체를
  생성하고 동작을 제어할 수 있습니다. 리플렉션은 객체를 동적으로 생성하거나, 클래스의 메소드나 필드에 접근하는 등의 용도로 사용됩니다.

프록시 패턴과 리플렉션은 모두 객체 지향 프로그래밍에서 객체를 다루는 기술로, 객체의 접근을 제어하거나 객체를 동적으로 생성하거나 동작을 제어하는 데 사용됩니다. 하지만, 프록시 패턴은 객체의 접근을 제어하는 데
사용되고, 리플렉션은 객체의 동적 생성과 동작 제어에 사용됩니다.

# 스프링 프레임워크에서 POJO는 무엇인가요?

스프링 프레임워크에서 POJO란 "Plain Old Java Object"의 약어로, **기본적인 자바 객체**를 의미합니다. 즉, 특정한 인터페이스나 클래스를 상속받지 않고 **순수하게 자바 객체로만 이루어진
것**을 의미합니다.

스프링 프레임워크에서 POJO는 DI(Dependency Injection)와 IoC(Inversion of Control)를 구현하기 위한 핵심 컴포넌트입니다. POJO는 DI와 IoC를 가능하게 하는 스프링
컨테이너에 의해 관리되며, 이를 통해 개발자는 객체 생성 및 의존성 주입에 대한 복잡한 작업을 자동화할 수 있습니다.

스프링 프레임워크에서 POJO를 사용하면, 각 객체가 자신의 역할에만 집중할 수 있으므로 코드의 가독성과 유지보수성이 좋아지고, 더 나은 테스트 코드 작성이 가능해집니다. 또한, POJO는 프레임워크에 종속되지
않으므로, 이식성이 높고 자유롭게 코드를 작성할 수 있습니다.
