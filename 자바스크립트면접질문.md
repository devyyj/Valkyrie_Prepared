...
# 어떤 파일이 함수를 export 한다면 그건 클로저인가요?

함수를 export하는 것 자체로는 클로저라고 할 수는 없습니다. 하지만 해당 함수가 자신이 정의된 렉시컬 스코프에 대한 참조를 유지하고 있는 경우, 그 함수를 사용하는 코드에서 클로저를 생성할 수 있습니다.

예를 들어, 다음과 같이 myFunction 함수를 정의하고 export하는 경우를 생각해보겠습니다.
...
위 코드에서 myFunction은 innerFunction을 반환합니다. 반환된 innerFunction은 myFunction이 정의된 렉시컬 스코프에 대한 참조를 유지하고 있기 때문에, 사용하는 코드에서
innerFunction을 호출하면 클로저가 생성됩니다.
...
var greet = myFunction();
greet('John'); // 'Hello, John!'
```

위 코드에서 myFunction을 호출하여 반환된 innerFunction을 greet 변수에 할당하고, greet를 호출하는 경우 myVariable의 값이 유지되면서 출력됩니다. 이때 greet가 참조하는
myFunction 내부의 myVariable은 클로저의 개념을 따르고 있습니다.

# 렉시컬 스코프란 무엇인가요?

렉시컬 스코프는 자바스크립트에서 **변수와 함수의 유효 범위를 결정하는 방식**을 말합니다. 이는 코드를 작성하는 동안 변수와 함수가 어떤 위치에서 선언되었는지에 따라 결정됩니다.

렉시컬 스코프는 소스 코드의 작성 순서에 기반하여 스코프를 정의합니다. 함수가 어떤 위치에서 선언되었는지에 따라 함수가 접근할 수 있는 변수의 범위가 결정되며, 변수는 자신이 선언된 위치에 가장 가까운 스코프에서
검색됩니다.
...
위의 코드에서 `inner` 함수는 `x` 변수를 출력합니다. 이때 `x` 변수는 `outer` 함수 내부에서 선언되었고, `inner` 함수 내부에서 사용되었습니다. 따라서 `inner` 함수는 자신이 선언된
위치에 가장 가까운 스코프인 `outer` 함수의 스코프에서 `x` 변수에 접근할 수 있습니다. 이와 같이 **렉시컬 스코프는 코드를 작성하는 시점에 정적으로 결정**되며, **실행 시점에는 변하지 않습니다.**

렉시컬 스코프는 자바스크립트의 스코프 체인 구조를 형성하고, 변수와 함수의 유효 범위를 보다 명확하게 결정하는 데 도움을 줍니다.

# 스코프 체인은 무엇인가요?

스코프 체인은 자바스크립트에서 **변수 및 함수를 검색하는 메커니즘**입니다. **렉시컬 스코프와 함께 사용**되며, 변수나 함수가 선언된 위치에 따라 스코프 체인이 형성됩니다.

스코프 체인은 중첩된 함수의 경우에 특히 중요합니다. **함수 내부에서 변수나 함수를 참조할 때, 자바스크립트 엔진은 현재 스코프에서 해당 식별자를 찾고, 없으면 상위 스코프로 이동하여 다시 검색합니다. 이런 식으로
계속 상위 스코프로 이동하며 검색하는 과정을 스코프 체인**이라고 합니다.

스코프 체인은 함수가 정의된 위치에서부터 시작하여 외부 함수, 전역 스코프까지 이동하며 변수나 함수를 찾습니다. 만약 식별자가 전역 스코프까지 검색되지 않으면 해당 식별자는 정의되지 않았다고 판단됩니다.

아래 예제를 통해 스코프 체인의 동작을 살펴보겠습니다:
...
위의 코드에서 `inner` 함수는 `x`, `y`, `z` 변수를 참조합니다. 스코프 체인을 통해 `z` 변수는 `inner` 함수의 스코프에서 바로 검색됩니다. 그 다음 `y` 변수는 `inner` 함수의 상위
스코프인 `outer` 함수의 스코프에서 검색됩니다. 마지막으로 `x` 변수는 전역 스코프에서 검색됩니다. 이렇게 스코프 체인을 따라가며 변수를 검색하는 것이 스코프 체인의 작동 방식입니다.

스코프 체인은 변수의 유효 범위를 결정하고, 함수가 동적으로 스코프를 결정하는 렉시컬 스코프 메커니즘을 지원합니다.
...
반면에 **프로토타입은 클래스와 달리 ES6 이전부터 사용되던 개념**입니다. 프로토타입은 **상속을 구현하기 위해 사용**되며, **자바스크립트에서 모든 객체는 프로토타입을 가지고 있습니다.** 프로토타입은 객체를 생성하는 함수에서
...
하지만, 클래스와 프로토타입은 기본적으로 동일한 개념으로, **클래스를 사용하면 코드를 더 직관적으로 작성할 수 있으며, 상속 구현도 더 쉽게 할 수 있습니다.** 또한, 클래스는 상속을 구현하는 방법으로서 더 권장되는
방법입니다.
